...
func (s *dialstate) newTasks(nRunning int, peers map[enode.ID]*Peer, now time.Time) []task {
	...
	// 判断已经建立和正在建立的连接的类型。如果是动态类型。那么需要建立动态链接数量减少。
	...
	// 如果fallbackInterval（20秒）内没有任何已创建连接，而且需要动态型连接数大于零，bootnodes数目也大于零，则尝试连接一个bootnodes
	// 此情况一般在测试网络或私有网络用，因为bootnodes的table可能存在大量坏节点
	...
	// 如果还不能创建足够动态链接。 那么创建一个discoverTask用来再网络上查找其他的节点。放入lookupBuf
