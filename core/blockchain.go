...
/*
以太坊提供了 "fast", "full", "light" 三种数据同步模式
	1. fast: 适用于仅提供rpc服务的节点。以太坊节点会从网络同步所有的区块头，区块体以及状态数据，
			但不对区块中的交易进行重放，只会对区块中的数据进行校验。
	2. full: 以太坊节点会从网络同步所有的区块头，区块体并重放区块中的交易以生成状态数据。
	3. light: 以太坊节点仅从网络中同步所有区块头，不去同步区块体，也不去同步状态数据，仅在需要相应区块和状态数据时从网络上获取。
*/
...
/*
	规范链：最长链?
	BlockChain 代表了规范链，BlockChain 管理着区块链的插入、回滚、重建等操作。
	插入区块，需要经过两个阶段的验证，验证由 Validator 处理。使用 Processor 来对处理区块交易，对区块状态的验证发生在第二阶段。两个阶段验证中出错将放弃插入该区块
	GetBlock 获得的区块不一定在规范链中
	GetBlockByNumber 获得的区块一定在规范链中
*/
...
// SetHead 会设置head为"规范链"的新区块头，删除高度大于新区块头的所有区块
...
//@ 通过验证的区块写入数据库，区块不一定在规范链中，故不一定更新区块头
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
	...
	// 如果新区块所在的链的总难度值，超过现在 "规范链" 的总难度值，则将新区块添加到 "规范链" 中
	// 重建规范链有两种：
	//   1. 新区块成为现有"规范链"的区块头
	//   2. 新区块所在的链替代现有的"规范链"
	reorg := externTd.Cmp(localTd) > 0 //是否要重建规范链
	currentBlock = bc.CurrentBlock()
	if !reorg && externTd.Cmp(localTd) == 0 {
		// Split same-difficulty blocks by number, then preferentially select
		// the block generated by the local miner as the canonical block.
		if block.NumberU64() < currentBlock.NumberU64() {
			// 若总难度值相等，新区块的高度矮，还是需要重建"规范链"
			reorg = true
		} else if block.NumberU64() == currentBlock.NumberU64() {
			// 若总难度值相等，高度也相等，则需要进一步判断
...
//@ 这里的重建"规范链"，是第二种情况，新区块所在的分支链成为新的"规范链"
// 重建只需要从共同祖先区块开始即可，非共同链部分，用 newChain和oldChain 表示
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	// 降低旧的"规范链"高度，与新的"规范链"高度一致
	if oldBlock.NumberU64() > newBlock.NumberU64() {
		// 老的区块高度 > 新的区块高度，但 老的区块难总度值 <= 新的区块总难度值
	...
	for _, tx := range types.TxDifference(deletedTxs, addedTxs) {
		// 当区块从规范的区块链移除，交易也会被移除，也就是所有交易始终都是当前"规范链"上的交易
		rawdb.DeleteTxLookupEntry(batch, tx.Hash())
	}
	// Delete any canonical number assignments above the new head
	// 删除所有高于新区块头高度的"规范链"的区块hash
	// case：旧的"规范链"高度 > 新的"规范链"高度
